...
The app is exposed the same way on the network it does join, and having the explicit alias does not remove it. Three names resolve to `ignis-http-ignis-1` on `tentacron-net`: the container name, `ignis` from the automatic service-name alias, and `ignis-http` from the explicit one. Only the last carries a guarantee. The container name changes with the project or the service; `ignis` is correct only while the HTTPS stack stays off the network, and would resolve non-deterministically between the two the day it does not. A caller reaching for `ignis` gets a working result today and no signal that it chose the name without the guarantee. `ignis-http` is the published name for that reason, and it is why the alias is transport-qualified rather than being the repository name alone.
